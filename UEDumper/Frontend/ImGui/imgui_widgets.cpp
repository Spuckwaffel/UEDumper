...
#define ColToVec(value1, value2, value3, value4) ImVec4((float)value1 / 255, (float)value2 / 255, (float)value3 / 255, (float)value4/255)

void ImGui::DrawCircleProgress(ImVec2 center, float radius, float percentage, ImU32 col, float thickness) {
    ImGuiWindow* window = GetCurrentWindow();

    percentage *= 1.01f;
    window->DrawList->PathClear();
    for (int i = -91; i < (int)((3.6 * percentage) - 90); i++) {
        float x = cos(i * acos(-1) / 180) * radius;
        float y = sin(i * acos(-1) / 180) * radius;
        window->DrawList->PathLineTo({ center.x + x, center.y + y });
    }
    window->DrawList->PathStroke(ColorConvertFloat4ToU32(ImVec4(0.28f, 0.56f, 1.00f, 1.00f)), false, thickness);
}
...
bool ImGui::SideBar(const char* label, ImFont* smallfont, ImFont* bigfont, const char* name, ImVec2 pos, ImVec2 size) {
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    //we initialize a ButtonBehavior for the entire bar, so we can use clicked for other further events
    bool hovered, held;
    bool clicked = ButtonBehavior(bb, id, &hovered, &held);

    GetWindowDrawList()->AddRectFilled(pos, ImVec2(pos.x + size.x, pos.y + size.y - 15), ImGui::GetColorU32(ColToVec(67, 56, 202, 255)), 0, 0);
    GetWindowDrawList()->AddRectFilled(ImVec2(pos.x, pos.y + size.y - 30), pos + size, ImGui::GetColorU32(ColToVec(67, 56, 202, 255)), 14, 0);
    GetWindowDrawList()->AddRectFilled(ImVec2(pos.x + size.x - 20, pos.y + size.y - 30), pos + size, ImGui::GetColorU32(ColToVec(67, 56, 202, 255)), 0, 0);


    GetWindowDrawList()->AddCircleFilled(ImVec2(size.x / 2, pos.y + 50), 30, ImGui::GetColorU32(ColToVec(224, 231, 255, 255)), 0);
    GetWindowDrawList()->AddText(bigfont, 40, ImVec2(size.x / 2 - 11, pos.y + 29), ImGui::GetColorU32(ImGuiCol_ButtonTextInverted), name);

    GetWindowDrawList()->AddLine(ImVec2(pos.x + 20, pos.y + 100), ImVec2(pos.x + size.x - 20, pos.y + 100), GetColorU32(ColToVec(224, 231, 255, 255)), 2);
    
    return false;
}

void ImGui::Userprofile(const char* label, ImFont* smallfont, const char* name, const char* description, bool* logout, const char* loaderver, int* clickon) {
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return;

    //PushStyleVar(ImGuiStyleVar_WindowBorderSize, 5);
    PushStyleColor(ImGuiCol_WindowBg, GetColorU32(ImGuiCol_ChildBg));
    Begin(label, NULL, ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoTitleBar);
    static int startX = 80;
    SetWindowSize(ImVec2(200, 150));
    
    SetWindowPos(ImVec2(startX, 50));
    SetCursorPos(ImVec2(15, 5));


    GetWindowDrawList()->AddRectFilled(ImVec2(startX + 1, 51), ImVec2(startX + 199, 85), GetColorU32(ColToVec(224, 231, 255, 255)), 15);
    GetWindowDrawList()->AddRectFilled(ImVec2(startX + 1, 70), ImVec2(startX + 199, 85), GetColorU32(ColToVec(224, 231, 255, 255)));
    GetWindowDrawList()->AddRect(ImVec2(startX + 1, 51), ImVec2(startX + 199, 199), GetColorU32(ColToVec(245, 246, 247, 255)), 15.f, 0, 4);
    Text(name);
    Dummy(ImVec2(10, 0));
    PushFont(smallfont);
    SetCursorPosX(15);
    Text(description);
    SetCursorPos(ImVec2(10, 85));
    if (Button("Sign out", ImVec2(100, 35))) {
    ...
    SetCursorPosX(20);
    TextDisabled(loaderver);
    PopFont();
    PopStyleColor();
    End();
}

bool ImGui::ReceivedFile(const char* name, const char* size, const ImVec2& size_arg, ImTextureID user_texture_id, int percent, int type, ImFont* smallfont) {
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    RenderNavHighlight(bb, id);
    const int small = 85;
    const const ImRect bb1 = ImRect(ImVec2(bb.Min.x + 20, bb.Min.y), ImVec2(bb.Max.x - 20, bb.Max.y));
    RenderTextClipped(ImVec2(bb.Min.x + ((bb.Max.x - bb.Min.x)/ 2) - (label_size.x / 2), bb.Min.y + 150), ImVec2(0, 0), name, NULL, &label_size, {0,0}, &bb1);
    PushFont(smallfont);
    const ImVec2 filesize_size = CalcTextSize(size, NULL, true);
    if (percent > 0) {
        const std::string spercent = std::to_string(percent) + "%";
        const ImVec2 spercent_size = CalcTextSize(spercent.c_str(), NULL, true);
        RenderTextClipped(ImVec2(bb.Min.x + ((bb.Max.x - bb.Min.x) / 2) - (spercent_size.x / 2), bb.Min.y + 50), ImVec2(0, 0), spercent.c_str(), NULL, &label_size, { 0,0 }, &bb1);
    }
    RenderTextClipped(ImVec2(bb.Min.x + ((bb.Max.x - bb.Min.x)/ 2) - (filesize_size.x / 2), bb.Min.y + 175), ImVec2(0, 0), size, NULL, &filesize_size, {0,0}, &bb1);
    PopFont();
    switch (type) {
        case 0: //nothing
        {
            SetMouseCursor(ImGuiMouseCursor_Hand);
            if (hovered) {
                window->DrawList->AddImage(user_texture_id, { bb.Min.x + small, bb.Min.y + small - 9 }, { bb.Max.x - small, bb.Max.y - small - 9 }, ImVec2(0, 0), ImVec2(1, 1), ColorConvertFloat4ToU32(ImVec4(0.28f, 0.56f, 1.00f, 1.00f)));
            }
            else {
                window->DrawList->AddImage(user_texture_id, { bb.Min.x + small, bb.Min.y + small - 9 }, { bb.Max.x - small, bb.Max.y - small - 9 });
        ...
        case 1: 
        {
            Spinner(GetFontSize(), 5, 50, { bb.Min.x + small + 8,bb.Min.y + small + 5 }, false);
            break;
        }
        case 2:
        {
            ...
            const float sz = 65;
            const ImVec2 pos = { bb.Min.x + small + 5, bb.Min.y + small - 3 };
            ...
            window->DrawList->PathStroke(ColorConvertFloat4ToU32(ColToVec(4, 222, 0, 255)), false, thickness);
            break;
        }
        case 3:
        {
            float sz = 65;
            const float thickness = 7;
            ImVec2 pos = { bb.Min.x + small, bb.Min.y + small - 9 };
            ImVec2 poscross2top = ImVec2(pos.x, pos.y);
            ImVec2 poscross2bottom = ImVec2(pos.x + sz, pos.y + sz);

            ImVec2 poscross1top = ImVec2(pos.x + sz, pos.y);
            ImVec2 poscross1bottom = ImVec2(pos.x, pos.y + sz);

            window->DrawList->PathLineTo(poscross1top); //bottom
            window->DrawList->PathLineTo(poscross1bottom); //top
            window->DrawList->PathStroke(ImGui::ColorConvertFloat4ToU32(ColToVec(255, 0, 0, 255)), false, thickness);
            window->DrawList->PathLineTo(poscross2top); //bottom
            window->DrawList->PathLineTo(poscross2bottom); //top
            window->DrawList->PathStroke(ImGui::ColorConvertFloat4ToU32(ColToVec(255, 0, 0, 255)), false, thickness);
            break;
        }
    }

    window->DrawList->AddCircle(ImVec2(bb.Max.x - (size_arg.x/2), bb.Max.y - (size_arg.x / 2)), (size_arg.x / 2) - 10, GetColorU32(ColToVec(232, 232, 232, 255)), 300, 7);
    DrawCircleProgress(ImVec2(bb.Max.x - (size_arg.x / 2), bb.Max.y - (size_arg.x / 2)), (size_arg.x / 2) - 10, percent, ColorConvertFloat4ToU32(ImVec4(0.28f, 0.56f, 1.00f, 1.00f)), 7);
    IMGUI_TEST_ENGINE_ITEM_INFO(id, label, g.LastItemData.StatusFlags);
    
    return pressed;
}

bool ImGui::FileBox(const char* label, const ImVec2& size_arg, ImTextureID user_texture_id, int percent, int type) {
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    const ImVec2 ImageOffset = ImVec2((bb.Min.x + (bb.Max.y - bb.Min.y)), bb.Min.y + 3);
    window->DrawList->AddRectFilled(bb.Min, bb.Max, GetColorU32(ColToVec(249, 250, 251, 255)), 4);
    window->DrawList->AddRect(bb.Min, bb.Max, GetColorU32(ColToVec(245, 246, 247, 255)), 4, 0, 2);
    window->DrawList->AddImage(user_texture_id, { bb.Min.x + 5, bb.Min.y + 5 }, { bb.Min.x + (bb.Max.y - bb.Min.y - 5), bb.Max.y - 5 });
    
    const float progress_bar_length = bb.Max.x - bb.Min.x - 70 - (ImageOffset.x - bb.Min.x);
    const const ImRect bb1 = ImRect(bb.Min, ImVec2(ImageOffset.x + progress_bar_length, bb.Max.y));
    RenderTextClipped(ImageOffset, ImVec2(ImageOffset.x, bb.Max.y), label, NULL, &label_size, { 0,0 }, &bb1);

    float progress = progress_bar_length * ((float)percent / 100.f);
    window->DrawList->AddLine(ImVec2(ImageOffset.x, bb.Max.y - 15), ImVec2(ImageOffset.x + progress_bar_length, bb.Max.y - 15), GetColorU32(ColToVec(232, 232, 232, 255)), 5);
    window->DrawList->AddLine(ImVec2(ImageOffset.x, bb.Max.y - 15), ImVec2(ImageOffset.x + progress, bb.Max.y - 15), ColorConvertFloat4ToU32(ImVec4(0.28f, 0.56f, 1.00f, 1.00f)), 5);

    switch (type) {
    case 0: //display percent
        RenderTextClipped(ImVec2(ImageOffset.x + progress_bar_length + 5, bb.Min.y + 10), bb.Max - style.FramePadding, std::string(std::to_string(percent) + "%").c_str(), NULL, &label_size, { 0,0 }, &bb);
        break;
    case 1: { //display check, file completely processed
        const float thickness = 5;
        const float sz = (bb.Max.y - bb.Min.y - 18) - thickness * 0.5f;
        const ImVec2 pos = ImVec2(ImageOffset.x + progress_bar_length + 15 + (thickness * 0.25f), bb.Min.y + 10 + (thickness * 0.25f));
        const float third = sz / 3.0f;
        const float bx = pos.x + third;
        const float by = pos.y + sz - third * 0.5f;
        window->DrawList->PathLineTo(ImVec2(bx - third, by - third));
    ...
    case 2: { //display loading indicator
        Spinner(GetFontSize() /2, 5, 50, ImVec2(ImageOffset.x + progress_bar_length + 20, bb.Min.y + 10), false);
        break;
        }
    case 3: { //display error
        float sz = 24.f;
        const float thickness = 5;
        ImVec2 pos = ImVec2(ImageOffset.x + progress_bar_length + 23, bb.Min.y + 13);
        ImVec2 poscross2top = ImVec2(pos.x, pos.y);
        ImVec2 poscross2bottom = ImVec2(pos.x + sz, pos.y + sz);
...
void ImGui::TitleBar(const char* label, ImVec2 size, ImVec2 pos, bool* clicked, bool* minimized, bool* closed, bool* held) {
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return;
    ...
    bool hovered = IsMouseHoveringRect(pos, pos + size);

    bool dheld = IsMouseDown(ImGuiMouseButton_Left);

    bool dclicked = IsMouseClicked(ImGuiMouseButton_Left);
    if (clicked)
        *clicked = dclicked;
    
    int minimizeoffset = 30;
    ImRect CloseButton(ImVec2(pos.x + 36, pos.y + 10), ImVec2(pos.x + 14, pos.y + 32));
    ImRect MinimizeButton(ImVec2(pos.x + 36 + minimizeoffset, pos.y + 10), ImVec2(pos.x + 14 + minimizeoffset, pos.y + 32));
    GetWindowDrawList()->AddRectFilled(pos, ImVec2(pos.x + size.x, pos.y + 30), ImGui::GetColorU32(ColToVec(232, 232, 232, 255)), 15, 0);
    GetWindowDrawList()->AddRectFilled(ImVec2(pos.x, pos.y + 15), ImVec2(pos.x + size.x, pos.y + size.y), ImGui::GetColorU32(ColToVec(232, 232, 232, 255)), 0, 0);
    GetWindowDrawList()->AddCircleFilled(ImVec2(pos.x + 25, pos.y + 21), 11, ImGui::GetColorU32(ColToVec(255, 0, 0, 255)), 0);
    GetWindowDrawList()->AddCircleFilled(ImVec2(pos.x + 25 + minimizeoffset, pos.y + 21), 11, ImGui::GetColorU32(ColToVec(255, 187, 0, 255)), 0);
    //GetForegroundDrawList()->AddRect(CloseButton.Min, CloseButton.Max, ImGui::GetColorU32(ColToVec(0, 0, 0, 255)));
    //GetForegroundDrawList()->AddRect(MinimizeButton.Min, MinimizeButton.Max, ImGui::GetColorU32(ColToVec(0, 0, 0, 255)));
    ...
    if (hovered) {
        if (isInside(pos.x + 25, pos.y + 21, 11, mpos.x, mpos.y)) {
            closehovered = true;
            dheld = false;
            if (dclicked && closed) {
                *closed = true;
        ...
        if (isInside(pos.x + 25 + minimizeoffset, pos.y + 21, 11, mpos.x, mpos.y)) {
            minhovered = true;
            dheld = false;
            if (dclicked && minimized) {
                *minimized = true;
            }
        }
    }
    ...
    if (minhovered) {
        GetWindowDrawList()->AddRectFilled(ImVec2(MinimizeButton.Min.x - 6, MinimizeButton.Min.y + 11), ImVec2(MinimizeButton.Max.x + 7, MinimizeButton.Max.y - 9), ImGui::GetColorU32(ColToVec(1, 1, 1, 255)), 2);
    }
    if (closehovered) {

        GetWindowDrawList()->AddCircleFilled(ImVec2(pos.x + 25, pos.y + 21), 3, ImGui::GetColorU32(ColToVec(1, 1, 1, 255)));

    }
    GetWindowDrawList()->AddText(ImVec2(pos.x + 100, pos.y + 7), ImGui::GetColorU32(ColToVec(1, 1, 1, 255)), label);
    if(held)
        *held = dheld;
}
...
void ImGui::Cross(float size, float thickness)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return;
    ...
    ImVec2 pos = ImVec2(window->DC.CursorPos.x + 5, window->DC.CursorPos.y + 5);
    ...
    ImVec2 poscross2top = ImVec2(pos.x, pos.y);
    ImVec2 poscross2bottom = ImVec2(pos.x + size, pos.y + size);

    ImVec2 poscross1top = ImVec2(pos.x + size, pos.y);
    ImVec2 poscross1bottom = ImVec2(pos.x, pos.y + size);
...
void ImGui::Spinner(float radius, float thickness, int num_segments, ImVec2 posi, bool IsRealWidget) {
    ...
    if (posi.x == 0 && posi.y == 0) {
        pos = ImVec2(window->DC.CursorPos.x + 5, window->DC.CursorPos.y);
    }
    else {
        pos = posi;
    }
    ...
    const float a_min = IM_PI * 2.0f * ((float)(abs(start))) / (float)num_segments;
    ...
    for (int i = 0; i < num_segments; i++) {
        ...
        window->DrawList->PathLineTo(ImVec2(centre.x + ImCos(a + g.Time * speed) * radius,
            centre.y + ImSin(a + g.Time * speed) * radius));
    }
...
bool ImGui::InputInt(const char* label, int* v, int step, int step_fast, ImGuiInputTextFlags flags)
{
    ...
    const char* format = (flags & ImGuiInputTextFlags_CharsHexadecimal) ? "%X" : "%d";
...
void   ImGui::SetTabItemFocus(const char* label)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
    if (window->SkipItems)
