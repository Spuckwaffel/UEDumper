...
namespace ImGui
{
    ...
    IMGUI_API void          SameLineEx(float spacing, float spacingy = 0.f, bool useNewY = false);  // call between widgets or groups to layout them horizontally. X position given in window coordinates.
    ...
    IMGUI_API bool          SideBar(const char* label, ImFont* smallfont, ImFont* bigfont, const char* name, ImVec2 pos, ImVec2 size);
    IMGUI_API void          Userprofile(const char* label, ImFont* smallfont, const char* name, const char* description, bool* logout, const char* loaderver, int* clickon);
    IMGUI_API bool          ReceivedFile(const char* name, const char* size, const ImVec2& size_arg, ImTextureID user_texture_id, int percent, int type, ImFont* smallfont);
    IMGUI_API void          TitleBar(const char* label, ImVec2 size, ImVec2 windowpos, bool* clicked = nullptr, bool* minimized = nullptr, bool* closed = nullptr, bool* held = nullptr);
    IMGUI_API bool          FileBox(const char* label, const ImVec2& size_arg, ImTextureID user_texture_id, int percent, int type);
    ...
    // Tab Bars, Tabs
    IMGUI_API void          SetTabItemFocus(const char* label);
