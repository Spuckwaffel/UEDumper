...
bool EngineCore::RUNAddMemberToMemberArray(EngineStructs::Struct& eStruct, const EngineStructs::Member& newMember)
{
	...
	for(int i = 0; i < eStruct.definedMembers.size(); i++)
	{
		...
		{
			//is the new member somehow interfering the next member
			if (newMember.offset + newMember.size > nextMember.offset)
			{
				windows::LogWindow::Log(windows::LogWindow::log_0, "CORE", "Add member failed: member is interfering other member (0x%X -> 0x%X -!- 0x%X)", newMember.offset, newMember.offset + newMember.size, nextMember.offset);
				return false;
			}
			//yep we found place
